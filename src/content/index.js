...
function record(entry) {
  if (!looksLikeImage(entry.name, entry.initiatorType)) return;
  ...
  if (entry.encodedBodySize) prev.bytes = entry.encodedBodySize;
...
function flush() {
  ...
  pending.clear();
  browser.runtime.sendMessage({ type: MSG.REPORT, batch }).catch(() => {
