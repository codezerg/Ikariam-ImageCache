...
await Promise.all(
  Array.from({ length: CONCURRENCY }, async () => {
    for (let job = queue.shift(); job; job = queue.shift()) {
      if (await run(job)) ok.add(job.path);
    }
  })
);
bar.end();
...
if (dead.length) {
  ...
  for (const f of dead.slice(0, 5)) console.log(`    ${f.url.split('/').pop()}`);
...
if (broke.length) {
  ...
  for (const f of broke.slice(0, 10)) console.log(`    ${f.url} (${f.reason})`);
  if (broke.length > 10) console.log(`    ...and ${broke.length - 10} more`);
  console.log('');
}
