...
import { isDenied, looksLikeImage, normalizeUrl } from '../src/lib/patterns.js';
...
for (const template of cfg.css) {
  ...
  for (const [, , ref] of css.matchAll(URL_RE)) {
    ...
    if (!looksLikeImage(abs)) continue;
    if (isDenied(abs, deny)) continue;
    found.add(abs);
    hits++;
  }
  ...
  JSON.stringify(
    {
      ...
      denyPatterns: deny.length ? deny : ['/avatar', '/captcha', '/temp/', '[.]php$'],
      urls: merged,
    },
    null,
    2
